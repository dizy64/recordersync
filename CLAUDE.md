...
- 원본/외부 오디오 볼륨은 각각 0.0~1.0이다. mix 기본값은 카메라 1.0, 외부
  `10^(-12/20)`, 외부 HP80이며 사용자가 두 볼륨과 HPF를 덮어쓸 수 있다. fallback은
  양쪽 1.0이고 replace에서는 카메라 볼륨을 적용하지 않는다.
- mix는 합산 뒤 `-16 LUFS`, `-1 dBTP`, stereo, `0.5 LU` 허용 오차의 음량 안전 정책을
  기본 적용한다. replace는 네 값을 모두 명시할 때만 활성화한다. 동적 처리를 자동
  적용하지 않고 최종 AAC 재검증 실패 파일을 게시하지 않는다.
