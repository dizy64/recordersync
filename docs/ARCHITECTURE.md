...
`process --mode fallback`에서만 영상 특징을 기본 5초 창으로 나눈다. 전체 매칭 후보나
...
2026-07-17 Apple Silicon 측정값은 일반 모드 총 31.623초, 영상당 p95 0.159초,
p99 0.160초, peak RSS 288.3MB였다. 전체 매칭이 실패하고 중간 단절 뒤 전역 재탐색하는
실제 `--partial` 입력은 총 160.084초, p95 0.805초, p99 0.810초, peak RSS
