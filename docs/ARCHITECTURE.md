...
콜백으로 전달한다. CLI는 이를 stderr에 한국어로 표시하고 stdout은 JSON 전용으로
유지한다. 인자 없이 실행하면 파일을 탐색하지 않고 최상위 도움말만 출력한다.

## 성능 기준

`scripts/benchmark_matcher.py`가 12시간 특징 타임라인과 60초 영상 200개를 생성한다.
2026-07-17 Apple Silicon 측정값은 총 31.645초, 영상당 p95 0.159초, p99 0.161초,
peak RSS 287.9MB였다. 기준은 총 600초와 2GB다. 숫자를 갱신할 때 하드웨어와 날짜를
함께 기록한다.
