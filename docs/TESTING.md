...
현재 기준은 단위 테스트 81개, E2E 3개, 단위 커버리지 88%다. 새 변경은 커버리지를 85% 아래로
...
| `test_matching.py` | 10 | 전체/부분/다중 구간, 반복/겹침, 무관 음원, drift, 다중 카메라 |
| `test_media.py` | 7 | 탐색, ffprobe 파싱, PCM, 실패, 조각 frame padding |
| `test_render.py` | 16 | 출력명·경로 안전, 다중 구간 폴백, crossfade, 프로파일, 원자 출력 |
| `test_pipeline.py` | 5 | 배치 분석, matched/partial 렌더 정책, 카메라음 없음 |
| `test_cli.py` | 21 | 도움말, 부분 opt-in, fallback, 사람용/JSON 리포트, 종료 코드 |
...
영상당 p95, peak RSS가 나온다. `--partial`은 전체 매칭 검증 뒤 부분 창 검증까지 포함한다.
알고리즘 변경 검토 시 두 모드를 측정하고 p99도 기록한다.

2026-07-17 Apple Silicon 재측정 기준:

```text
mode              total      p95      p99      peak RSS
default          31.698 s   0.160 s  0.169 s   288.2 MB
--partial        33.758 s   0.170 s  0.175 s   288.6 MB
```
