...
현재 기준은 단위 테스트 129개, E2E 3개, 단위 커버리지 90%다. 새 변경은 커버리지를 85% 아래로
...
| `test_matching.py` | 13 | 전체 조기 반환, 부분/다중 구간, 전역 재탐색 상한, 입력 검증, drift, 다중 카메라 |
| `test_media.py` | 7 | 탐색, ffprobe 파싱, PCM, 실패, 조각 frame padding |
| `test_render.py` | 16 | 출력명·경로 안전, 다중 구간 폴백, crossfade, 프로파일, 원자 출력 |
| `test_pipeline.py` | 8 | 배치 분석, 공통 렌더 정책, 추천 전용 필터, 잘못된 구간의 영상별 오류 처리 |
| `test_cli.py` | 32 | 도움말, 기본 부분 분석, 분석 재사용, 배치 명령 추천, fallback, 리포트, 종료 코드 |
| `test_analysis_plan.py` | 6 | 분석 입력 round-trip, 지문·디렉터리·버전 검증, 언어 독립성 |
| `test_api.py` | 6 | 외부 소비자용 세션·매칭·영상별 오류 격리·다중 세션 렌더 계획 API |
| `test_report.py` | 11 | JSON v2, 부분 구간/사용률, 추천 명령, 한국어·영어 목록, 실패 사유 |
| `test_recommendation.py` | 13 | 영상별/배치 replace·fallback 추천과 보수적인 보류 경계 |
| `test_check_markdown_links.py` | 4 | 로컬 링크, 외부 URL, 코드 블록, 저장소 이탈, CLI 오류 목록 |
| `test_report_json_schema.py` | 4 | Draft 2020-12 적합성, 실제 일반/재사용 payload, 문서 예시, 잘못된 상태 거부 |
