...
```text
분석 결과: 1/2개 매칭 (50.0%)
- clip-001.mov | 매칭 여부: 성공 | 매칭률: 94.0% | 추천: replace
...
기대 시작점은 약 3.00초이며 출력 해상도는 원본과 같은 320×240이고 `r_frame_rate`와 `avg_frame_rate`는 입력과 같은 `30000/1001`이다. 이 수동 smoke는 CFR 입력만 검증하므로 VFR 보존 근거로 사용하지 않는다. `bash scripts/test-e2e.sh`는 전반 30fps·후반 15fps의 합성 VFR 입력을 별도로 렌더해 전체 프레임 타임스탬프를 비교한다. 스마트폰 회전 메타데이터는 해당 특성을 가진 별도 입력으로 표시 방향이 유지되는지 확인한다. 확인 후 임시 디렉터리는 삭제해도 된다.
