...
def test_다중_대역_특징_생성은_유한한_정규화_대역을_반환한다() -> None:
    ...
    samples = (np.sin(2 * np.pi * 220 * time) + 0.5 * np.sin(2 * np.pi * 1_200 * time)).astype(np.float32)
...
def test_부분_매칭은_중간_불일치_양쪽을_서로_다른_세션으로_찾는다() -> None:
    ...
    assert result.status is MatchStatus.PARTIAL
    assert [
        (segment.session_id, segment.video_start_seconds, segment.duration_seconds) for segment in result.segments
    ] == [
...
def test_전체_매칭이_성공하면_부분_구간을_추가로_탐색하지_않는다() -> None:
    rng = np.random.default_rng(41)
    video = _standardize(rng.normal(size=(6, 100)).astype(np.float32))
    session = rng.normal(scale=0.01, size=(6, 300)).astype(np.float32)
    session[:, 100:200] += video
    timeline = FeatureTimeline("session-001", _standardize(session), 0.05)

    with patch.object(matching_module, "_find_partial_segments") as find_partial:
        result = match_video_features(
            Path("clip.mov"),
            duration_seconds=5.0,
            video_features=video,
    ...
    assert result.status is MatchStatus.MATCHED
    find_partial.assert_not_called()
