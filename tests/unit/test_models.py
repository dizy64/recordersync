...
def test_부분_매칭은_겹치거나_영상_밖으로_나간_구간을_거부한다() -> None:
    ...
    with pytest.raises(ValueError, match="cover the full video"):
        AudioMatch(
            Path("clip.mov"),
            10.0,
            MatchStatus.MATCHED,
            segments=(AudioMatchSegment("session-001", 1.0, 10.0, 8.0),),
        )
    ...
    with pytest.raises(ValueError, match="cover the full video"):
        AudioMatch(
            ...
            MatchStatus.MATCHED,
            segments=(
                AudioMatchSegment("session-001", 0.0, 10.0, 5.0),
                AudioMatchSegment("session-001", 5.0, 15.0, 5.0),
            ),
        )
